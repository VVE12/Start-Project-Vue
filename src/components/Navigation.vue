<template>
    <nav class="navbar navbar-expand-lg navbar-dark fixed-top" id="mainNav">
        <div class="container">
            <router-link class="navbar-brand js-scroll-trigger" to="/">Start Project Vue</router-link>
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav text-uppercase ml-auto">
                    <li class="nav-item" v-for="(linkObj, ind) in navList" :key="ind">
                        ...
                        <router-link class="nav-link js-scroll-trigger" v-else :to="linkObj.path">{{ linkObj.name }}</router-link>
                    </li>
                </ul>
            </div>
        </div>
